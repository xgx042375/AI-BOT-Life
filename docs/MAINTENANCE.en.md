...
| Start options | **Telegram channel** (international IM, added 2026-09-12; **no UI switch yet** — edit `.env` by hand, see leftover A16) | `.env` `TELEGRAM_ENABLED` / `TELEGRAM_BOT_TOKEN` / `TELEGRAM_OWNER_ID` / `TELEGRAM_PROXY` / `TELEGRAM_API_BASE` | restart |
...
| Model & API | **API Key reveal toggle** (2026-09-12, A19 ①) | nothing is persisted: the real value lives in memory, the box shows dots while masked, and **saving / fetching models takes the real value** (so dots can never be written into `.env`) | immediate (auto-reveals on focus, re-masks on blur) |
| Model & API | **Hide the Settings page** (2026-09-12, A19 ②; for streaming/recording) | the `hideCfg` key of `data/launcher.json` | immediate (the top-bar "Settings" entry collapses, and a `cfg` command coming from a skin is blocked too); restore with **Ctrl+Shift+H** |
...
| P | **plugin registration and the synthetic-channel red line** (added 2026-09-12, when the Telegram channel landed): ① a `plugins/<dir>/` directory ⟷ the `[tool.nonebot].plugins` list in `pyproject.toml` — **a directory that exists but is not registered means the whole plugin silently does nothing** (actually hit with livesource: the launcher wrote the switch, the docs described it, the entire chain had zero consumers and **zero errors**); the reverse "ghost registration" (listed but the directory is gone) is reported too ② **a synthetic-event injection point must carry the synthetic-round mark** — a file containing `handle_event(` must also contain `set_synthetic_round(` or `_SYNTHETIC[` (without it, synthetic channels such as Telegram/GAL **act on the real QQ account**: profile edits, typing status; webgal plan §5 item #4's red line). Both slices ship positive/negative self-tests |
